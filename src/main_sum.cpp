...
void run(int argc, char** argv)
{
    ...
    std::vector<double> memory_write_times;

    for (int iter = 0; iter < 10; ++iter) {
        timer t;
        input_gpu.writeN(values.data(), n);
        memory_write_times.push_back(t.elapsed());
    }

    double memory_size_gb = sizeof(unsigned int) * n / 1024.0 / 1024.0 / 1024.0;
    std::cout << "sum median effective executor memory write speed: " << memory_size_gb / stats::median(memory_write_times) << " GB/s" << std::endl;
